...
import { readClaims, listClaimFiles } from "./claims-storage.js";
...
export async function verifyClaimFile(path: string): Promise<IntegrityReport> {
  const errors: IntegrityError[] = [];
  let totalEntries = 0;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!verifyEntryHash(claim as unknown as Record<string, unknown>)) {
      errors.push({
        file: path,
        line: i + 1,
        type: "hash_mismatch",
        detail: `Claim ${claim.id} hash does not match computed hash`,
      });
    }
  }

  // Verify chain linkage
  const result = await readClaims(path);
  if (result.ok && result.value.length > 1) {
    const chainResult = verifyChain(result.value);
    if (!chainResult.valid && chainResult.brokenAt !== undefined) {
      const brokenEntry = result.value[chainResult.brokenAt];
      errors.push({
        ...
        line: chainResult.brokenAt + 1,
        type: "chain_broken",
        detail: `Claim ${brokenEntry?.id ?? "unknown"} prevHash does not match previous entry hash`,
      });
    }
  }
...
export async function verifyAllClaims(claimsDir: string): Promise<IntegrityReport> {
  const filesResult = await listClaimFiles({ claimsDir });
  if (!filesResult.ok) {
    return {
      valid: false,
      totalEntries: 0,
      filesChecked: 0,
      errors: [{ file: claimsDir, line: 0, type: "parse_error", detail: filesResult.error }],
    };
  }
  ...
  for (const file of files) {
    const report = await verifyClaimFile(file);
