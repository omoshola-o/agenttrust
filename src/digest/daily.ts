...
/**
 * Generate a daily markdown digest from collected data.
 */
export function generateDailyDigest(data: DigestData): string {
  const lines: string[] = [];
  const dateLabel = formatDate(data.period.from);

  // Header
  lines.push(`# AgentTrust Daily Digest \u2014 ${dateLabel}`);
  lines.push("");
  ...
  const totalAlerts =
    data.activity.byRiskLevel.critical +
    data.activity.byRiskLevel.high +
    data.activity.byRiskLevel.medium;
  lines.push(
    `- **Risk alerts**: ${data.highlights.ruleMatches.length} (${data.activity.byRiskLevel.critical} critical, ${data.activity.byRiskLevel.high} high, ${data.activity.byRiskLevel.medium} medium)`,
  );
  lines.push("");
  ...
  if (data.highlights.ruleMatches.length > 0) {
    ...
    for (const { entry, matches } of data.highlights.ruleMatches) {
      ...
      lines.push(
        `### ${emoji} ${sev}: ${matches[0]!.reason} (${time})`,
      );
      lines.push(`Agent performed \`${entry.action.type}\` on \`${truncate(entry.action.target, 60)}\`.`);
      lines.push(`**Rule**: ${ruleIds}`);

      if (matches[0]!.evidence) {
        const evidenceParts = Object.entries(matches[0]!.evidence)
          .map(([k, v]) => `${k}=${v}`)
          .join(", ");
  ...
  if (data.timeline.length > 0) {
    ...
    for (const item of data.timeline) {
      ...
      lines.push(
        `| ${time} | ${item.action} | ${truncate(item.target, 40)} | ${item.risk} ${emoji} |`,
      );
    }
    lines.push("");
  }
